...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state) {
		case MJRefreshStatePulling: // 松开可立即刷新
        {
            // 设置文字
            //ybyaoRresh
            _statusLabel.text = NSLocalizedString(MJRefreshHeaderReleaseToRefresh,@"MJRefreshHeaderView.ｍ");
            // 执行动画
		...
		case MJRefreshStateNormal: // 下拉可以刷新
        {
            // 设置文字
            //ybyao Rresh
			_statusLabel.text = NSLocalizedString(MJRefreshHeaderPullToRefresh,@"MJRefreshHeaderView.ｍ");
            // 执行动画
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
		...
		case MJRefreshStateRefreshing: // 正在刷新中
        {
            // 设置文字
            //ybyaoRresh
            _statusLabel.text = NSLocalizedString(MJRefreshHeaderRefreshing,@"MJRefreshHeaderView.m");
            // 执行动画
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
                _arrowImage.transform = CGAffineTransformIdentity;
                // 1.增加65的滚动区域
