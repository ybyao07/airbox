...
- (void)initAttributes
{
    ...
    if(_type == kRMClosedIndicator)
    {
        ...
        _strokeColor = [UIColor clearColor];//ybyao07
        _closedIndicatorBackgroundStrokeColor = [UIColor colorWithRed:180/255.0f green:189/255.0f blue:180/255.0f alpha:1];
        _coverWidth = 4.0;
...
- (void)addDisplayLabel
{
    ...
    if ([MainDelegate isLanguageEnglish]) {
        self.displayLabel = [[RMDisplayLabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.bounds)/2 - 60/2)-10, (CGRectGetHeight(self.bounds)/2 - 30/2)-10, 80, 40)];

        self.displayLabel.text = @" Download \n succeeded";
        self.displayLabel.numberOfLines=0;
        self.displayLabel.font = [UIFont fontWithName:@"EuphemiaUCAS" size:11];

    }else{
        self.displayLabel = [[RMDisplayLabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.bounds)/2 - 60/2), (CGRectGetHeight(self.bounds)/2 - 30/2), 60, 30)];
        self.displayLabel.font = [UIFont fontWithName:@"EuphemiaUCAS" size:15];
        self.displayLabel.text = NSLocalizedString(@"下载成功", @"RMDownloadIndicator.m") ;
