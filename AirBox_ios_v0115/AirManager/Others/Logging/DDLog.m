...
+ (void)queueLogMessage:(DDLogMessage *)logMessage asynchronously:(BOOL)asyncFlag
{
    ...
    dispatch_semaphore_wait(queueSemaphore, DISPATCH_TIME_NOW);
...
+ (void)lt_log:(DDLogMessage *)logMessage
{
    ...
    if (numProcessors > 1)
    {
        ...
        dispatch_group_wait(loggingGroup, DISPATCH_TIME_NOW);
    }
    else
    {
...
- (id <DDLogFormatter>)logFormatter
{
    ...
    //
    // DDLogCVerbose(@"log msg 1");
    // DDLogCVerbose(@"log msg 2");
    // [logger setFormatter:myFormatter];
    // DDLogCVerbose(@"log msg 3");
    //
