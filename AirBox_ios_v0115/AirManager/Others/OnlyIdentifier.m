...
NSString *const kOnlyIdentifierErrorDomain = @"com.samsoffes.OnlyIdentifier";

NSString *const kOnlyIdentifierAccountKey = @"acct";
NSString *const kOnlyIdentifierCreatedAtKey = @"cdat";
NSString *const kOnlyIdentifierClassKey = @"labl";
NSString *const kOnlyIdentifierDescriptionKey = @"desc";
NSString *const kOnlyIdentifierLabelKey = @"labl";
NSString *const kOnlyIdentifierLastModifiedKey = @"mdat";
NSString *const kOnlyIdentifierWhereKey = @"svce";
NSString *const kOnlyIdentifierAccount = @"com.thundersoft.airmanager";
NSString *const kOnlyIdentifier = @"identifier";

#if __IPHONE_4_0 && TARGET_OS_IPHONE
CFTypeRef OnlyIdentifierAccessibilityType = NULL;
#endif

@interface OnlyIdentifier ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation OnlyIdentifier
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = OnlyIdentifierErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kOnlyIdentifierErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (OnlyIdentifierAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return OnlyIdentifierAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (OnlyIdentifierAccessibilityType) {
		CFRelease(OnlyIdentifierAccessibilityType);
	}
	OnlyIdentifierAccessibilityType = accessibilityType;
}
#endif
...
+ (NSString *)identifier
{
    NSString *identifier = [self passwordForService:kOnlyIdentifier account:kOnlyIdentifierAccountKey];
    if ( 0 >= [identifier length] || nil == identifier  || [identifier isEqualToString:@""] || [identifier isEqual:[NSNull null]]) {
        NSString *cfuuidString;
        for (int i=0; i<3; i++) {
            CFUUIDRef cfuuid = CFUUIDCreate(kCFAllocatorDefault);
            cfuuidString = (NSString*)CFBridgingRelease(CFUUIDCreateString(kCFAllocatorDefault, cfuuid));
            if ( 0 >= [cfuuidString length] || nil == cfuuidString  || [cfuuidString isEqualToString:@""] || [cfuuidString isEqual:[NSNull null]])
            {
                continue;
            }
            else
            {
                [self setPassword:cfuuidString forService:kOnlyIdentifier account:kOnlyIdentifierAccountKey];
                break;
            }
        }
    }
    return [self passwordForService:kOnlyIdentifier account:kOnlyIdentifierAccountKey];
}
